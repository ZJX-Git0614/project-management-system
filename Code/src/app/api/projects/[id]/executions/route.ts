...
const validateTaskIds = async (projectId: string, taskIds: string[]) => {
  ...
  if (tasks.length !== taskIds.length) return null;
  return taskIds;
};
...
export async function GET(req: NextRequest, { params }: RouteContext) {
  ...
  return ok(executions.map(serializeProjectExecution));
}
...
export async function POST(req: NextRequest, { params }: RouteContext) {
  ...
  if (await validateTaskIds(projectId, taskIds) === null) return err("关联任务必须属于当前项目");
    ...
    const created = await tx.projectExecution.create({
      data: {
        ...
        taskLinks: taskIds.length
          ? { createMany: { data: taskIds.map((ganttTaskId) => ({ ganttTaskId })) } }
          : undefined,
      },
      include: projectExecutionInclude,
    });
