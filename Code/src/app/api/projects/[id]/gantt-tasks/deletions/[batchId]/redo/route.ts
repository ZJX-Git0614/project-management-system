...
import { GanttRevisionConflictError, redoGanttTaskDeletionBatch } from "@/lib/gantt-task-service";
...
export async function POST(
  ...
  try {
    const result = await redoGanttTaskDeletionBatch({
      projectId: id,
      batchId,
    ...
    return err(error instanceof Error ? error.message : "取消撤销失败");
  }
}
