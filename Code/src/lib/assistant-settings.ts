...
export const ASSISTANT_TOOL_CATALOG: readonly AssistantToolDefinition[] = [
  ...
  { id: "gantt.resource.optimize", version: 1, label: "应用资源优化排期", description: "基于跨项目负责人时间冲突生成候选方案；用户选定并确认后，按同一快照串行调整当前项目未开始且非手工排程的叶子任务日期", routingHints: ["询问资源优化建议时先读取资源分析并解释候选方案，不直接写入；只有用户明确采用某个方案时才调用本工具", "candidateKind：MINIMAL_CHANGE=最少改动，EARLIEST_FINISH=最早完成，ON_TIME=按期优先"], riskLevel: "MEDIUM", permissions: ["project-gantt:edit"], inputSchema: { additionalProperties: false, properties: { candidateKind: { type: "string", required: true, enum: ["MINIMAL_CHANGE", "EARLIEST_FINISH", "ON_TIME"] }, revision: { type: "number", required: true, minimum: 0 }, snapshotHash: { type: "string", required: true, minLength: 1, maxLength: 128 } } }, outputSchema: { kind: "NAVIGATION", required: ["message", "changedTaskCount", "navigateUrl"] }, idempotency: "KEYED_WRITE", retryPolicy: { maxAttempts: 1, retryableErrorCodes: [] }, verifier: "DATABASE_STATE" },
