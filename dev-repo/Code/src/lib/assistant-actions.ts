...
const ASSISTANT_EXECUTION_STALE_MS = 15 * 60 * 1000;
...
export const executeAssistantAction = async (action: AssistantActionRun, user: AuthenticatedUser) => {
  ...
  if (action.status === "EXECUTING") {
    const startedAt = action.confirmedAt?.getTime() ?? action.updatedAt.getTime();
    if (startedAt <= Date.now() - ASSISTANT_EXECUTION_STALE_MS) {
      await prisma.assistantActionRun.updateMany({
        where: { id: action.id, userId: user.userId, status: "EXECUTING" },
        data: {
          status: "FAILED",
          errorCode: "TEMPORARY_DEPENDENCY",
          errorMessage: "助手动作执行超时或服务在执行期间中断，请重新生成操作预览后再试。",
        },
      });
