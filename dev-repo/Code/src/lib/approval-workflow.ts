export const APPROVAL_BUSINESS_TYPES = {
  ...
  PROJECT_EXECUTION_EXIT_GATE: "PROJECT_EXECUTION_EXIT_GATE",
...
export const APPROVAL_BUSINESS_TYPE_LABEL: Record<string, string> = {
  ...
  PROJECT_EXECUTION_EXIT_GATE: "项目执行阶段出口 Gate",
...
export const DEFAULT_APPROVAL_WORKFLOWS: ApprovalWorkflowDraftInput[] = [
  ...
  },
  {
    ...
    moduleKey: "execution",
    name: "项目执行阶段出口 Gate 审批",
    description: "阶段范围内任务、事项和高风险满足出口条件后，由项目经理审批放行。",
    triggerPermissionKey: "project-execution:gate-request",
    completionHandlerKey: "APPROVE_PROJECT_EXECUTION_EXIT_GATE",
    nodes: [
      {
        nodeKey: "project-manager-approval",
        nodeOrder: 1,
...
export const approvalBusinessIdForProjectExecutionExitGate = (projectId: string, executionId: string) =>
  `${projectId}:execution-exit-gate:${executionId}`;
