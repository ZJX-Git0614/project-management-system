...
const validateScopeLinks = async (
  ...
  if (!scopeLinks.length) return true;
  const count = await prisma.projectGanttTask.count({
    where: { projectId, id: { in: scopeLinks.map((link) => link.ganttTaskId) } },
  });
  return count === scopeLinks.length;
};
...
export async function PUT(req: NextRequest, { params }: RouteContext) {
  ...
  const hasScopeLinks = Object.prototype.hasOwnProperty.call(body, "scopeLinks") || Object.prototype.hasOwnProperty.call(body, "taskIds");
  const scopeLinks = hasScopeLinks
    ? normalizeExecutionScopeLinks(body.scopeLinks, body.taskIds)
    : current.taskLinks.map((link) => ({
      ganttTaskId: link.ganttTaskId,
      relationType: link.relationType === "SUBTREE" ? "SUBTREE" as const : "DIRECT" as const,
    }));
  if (!await validateScopeLinks(projectId, scopeLinks)) return err("关联任务必须属于当前项目");
  if (current.status === "COMPLETED" && hasScopeLinks) {
    return err("阶段已通过出口 Gate，不允许直接调整范围；请先重新开启阶段并重新评估", 409, "EXECUTION_GATE_PASSED");
  }
  if (body.status === "COMPLETED" && current.status !== "COMPLETED") {
    return err("阶段完成必须通过出口 Gate 放行", 409, "EXECUTION_GATE_REQUIRED");
  }
  ...
  const execution = await prisma.$transaction(async (tx) => {
    const updated = await tx.projectExecution.update({
      ...
      data: {
        ...
        taskLinks: hasScopeLinks
          ? {
              deleteMany: {},
              ...(scopeLinks.length ? { createMany: { data: scopeLinks } } : {}),
            }
          : undefined,
      },
      include: projectExecutionInclude,
