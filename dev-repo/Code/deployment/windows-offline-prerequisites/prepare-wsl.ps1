...
$wslInstaller = Join-Path $PSScriptRoot "wsl.2.7.10.0.x64.msi"
if (-not (Test-Path $wslInstaller)) {
  throw "缺少 WSL2 安装包：$wslInstaller"
}
...
Write-Host ""
Write-Host "WSL2 准备完成。请立即重启 Windows。" -ForegroundColor Green
Write-Host "重启后运行 2-install-docker-desktop.bat。"
